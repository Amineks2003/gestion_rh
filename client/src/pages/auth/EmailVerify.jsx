...
const EmailVerify = () => {
  ...
  const onSubmitHandler = async (e) => {
    try {
      ...
      const { data } = await axiosInstance.post('/auth/verify-account', { otp, userId: userData._id })
      if (data.success) {
  ...
  const onResend = async () => {
    try {
      const { data } = await axiosInstance.post('/auth/send-verify-otp', { userId: userData?._id });
  ...
  return (
    ...
    >
      <form onSubmit={onSubmitHandler} className="bg-white/40 backdrop-blur-lg border border-blue-200 p-8 rounded-2xl shadow-2xl w-full sm:w-96 text-gray-700 text-base transition-all duration-500 hover:scale-[1.03]">
        ...
        <button className='w-full py-2 px-4 rounded-full bg-blue-400 hover:bg-blue-500 text-white font-semibold'>Vérifier l'émail</button>
