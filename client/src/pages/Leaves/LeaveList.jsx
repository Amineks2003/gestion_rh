...
const LeaveList = () => {
  ...
  return (
    <div className="min-h-screen py-10 flex justify-center bg-[#f6f7f0]">
      {/* CHANGED: max-w-6xl -> max-w-7xl 
         (You can also use w-11/12 for even wider) 
      */}
              ...
              {filteredLeaves.length === 0 ? (
                ...
                filteredLeaves.map((leave) => (
                  <tr key={leave._id} className="border-b hover:bg-gray-50 transition">
                    ...
                    <td className="py-4 px-6 text-center">
                      <div className="truncate max-w-[200px] mx-auto text-sm" title={leave.reason}>
                        {leave.reason}
                      </div>
                    </td>
                      ...
                      {leave.status === 'Pending' ? (
                        <div className="flex justify-center gap-2">
                          <button 
                            ...
                            className="bg-green-500 hover:bg-green-600 text-white px-3 py-1 rounded-lg text-xs font-semibold shadow transition"
                          >
                            Approve
                          </button>
                          <button 
                            onClick={() => handleStatusUpdate(leave._id, 'Rejected')}
                            className="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded-lg text-xs font-semibold shadow transition"
                          >
                            Reject
                          </button>
                        </div>
